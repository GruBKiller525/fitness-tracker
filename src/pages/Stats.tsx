...
type Tab = 'ejercicio' | 'actividad' | 'cuerpo';

export function Stats() {
  const [tab, setTab] = useState<Tab>('ejercicio');

  const labels: Record<Tab, string> = { ejercicio: 'Ejercicio', actividad: 'Actividad', cuerpo: 'Cuerpo' };

  return (
    <PageShell title="Estadísticas">
      <div className="flex gap-1 mb-4">
        {(['ejercicio', 'actividad', 'cuerpo'] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
